...
describe Nezu::Generators do
  describe '#template_to' do
    it 'should create a (parsed) copy in the app dir' do
      ...
      File.exists?(File.join(Nezu::Generators::GENERATOR.destination_root, '/test_file')).should be_true
    end

    it 'should parse .tt files using erb' do
      template_to('test_file')
      File.read(File.join(Nezu::Generators::GENERATOR.destination_root, '/test_file')).match(/Hi there/).should_not be_nil
    end

    after do
      %x(rm -rf #{Nezu::Generators::GENERATOR.destination_root})
    end
  end
